...
class GetMailCommand (ProxyPlugin.CommandParser):
    def parse(self, cmdEvent):
        playerOid = cmdEvent.getObjectOid()
        cmd = cmdEvent.getCommand()
        splitCmd = cmd.split(" ")
        AgisInventoryClient.getMail(playerOid)
        return None  

class MailReadCommand (ProxyPlugin.CommandParser):
    def parse(self, cmdEvent):
        playerOid = cmdEvent.getObjectOid()
        cmd = cmdEvent.getCommand()
        splitCmd = cmd.split(" ")
        Log.debug("MAIL: splitCMD[1]=" + splitCmd[1])
        mailID = OID.fromString(splitCmd[1])
        AgisInventoryClient.mailRead(playerOid, mailID)
        return None

class TakeMailItemCommand (ProxyPlugin.CommandParser):
    def parse(self, cmdEvent):
        playerOid = cmdEvent.getObjectOid()
        cmd = cmdEvent.getCommand()
        splitCmd = cmd.split(" ")
        Log.debug("MAIL: splitCMD[1]=" + splitCmd[1])
        mailID = OID.fromString(splitCmd[1])
        AgisInventoryClient.takeMailItem(playerOid, mailID)
        return None

class DeleteMailCommand (ProxyPlugin.CommandParser):
    def parse(self, cmdEvent):
        playerOid = cmdEvent.getObjectOid()
        cmd = cmdEvent.getCommand()
        splitCmd = cmd.split(" ")
        Log.debug("MAIL: splitCMD[1]=" + splitCmd[1])
        mailID = OID.fromString(splitCmd[1])
        AgisInventoryClient.deleteMail(playerOid, mailID)
        return None

class SendMailCommand (ProxyPlugin.CommandParser):
    def parse(self, cmdEvent):
        playerOid = cmdEvent.getObjectOid()
        cmd = cmdEvent.getCommand()
        splitCmd = cmd.split(">>:<<")
        ...
        message = splitCmd[3]
        itemOID = splitCmd[4]
        AgisInventoryClient.sendMail(playerOid, recipient, subject, message, itemOID)
        return None  
...
proxyPlugin.registerCommand("/getMail", GetMailCommand())
proxyPlugin.registerCommand("/mailRead", MailReadCommand())
proxyPlugin.registerCommand("/takeMailItem", TakeMailItemCommand())
proxyPlugin.registerCommand("/deleteMail", DeleteMailCommand())
proxyPlugin.registerCommand("/sendMail", SendMailCommand())
