...
public class AtavismCommand
{
	...
	public static void HandleCommand (string message)
	{
		...
		Logger.LogDebugMessage ("HandleCommand: " + message);
		if (message.Length == 0)
			return;
		...
		Logger.LogDebugMessage ("num args: " + tokens.Length + " with command: " + command);
	...
	public static void SendCommandToServer(string command, long targetOid) {
		...
		Logger.LogDebugMessage("Sending command to server");
	}
