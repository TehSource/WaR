...
public class SDETargeting : MonoBehaviour {
	...
	void LateUpdate () {
		...
		//if (Physics.SphereCast (cam.position + cam.forward * skipZone, sphereCastRadius , cam.forward, out hit, maxTargetDistance, mask) && hit.collider.transform.name != player.name) {
			...
			if (worldObj != null)
				mobObj = worldObj as MobNode;
			if (mobObj != null && mobObj.PropertyExists("health")){
				var health = (int)mobObj.GetProperty("health");
				if (health > 0)
					Client.Instance.WorldManager.TargetId = mobObj.Oid;
			} else {
				if (LoseLock)
				Client.Instance.WorldManager.TargetId = -1;
			}
		} else {
	...
	public void TabTarget(){
		...
		foreach (long worldObjOID in worldObjOIDs) {
			ObjectNode worldObj = ClientAPI.WorldManager.GetObjectNode(worldObjOID);
			if (worldObj != null && worldObj.Oid != ClientAPI.GetPlayerOid() && worldObj.CheckBooleanProperty ("attackable")){
				var mobObj = worldObj as MobNode;
				if (mobObj != null)
		...
		if (Client.Instance.WorldManager.TargetId == -1) {
			tabIndex = 0;
		...
		Client.Instance.WorldManager.TargetId = TabTargetNearestEnemy (tabIndex, worldObjects);
		
	}

	public static List<ObjectNode> TestVisible(List<ObjectNode> entities, float maxDistance){
	...
	public static long TabTargetNearestEnemy (int tabIndex, List<ObjectNode> wo)
	{
		List<ObjectNode> worldObjects = wo;
		Vector3 playerPos = ClientAPI.GetPlayerObject ().Position;
		worldObjects.Sort ((ObjectNode x, ObjectNode y) => {
