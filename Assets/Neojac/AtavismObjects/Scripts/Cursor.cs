...
public class Cursor : MonoBehaviour
{
	...
	public void PickupBagItem (int bagNum, int slotNum, AtavismInventoryItem item)
	{
		if (CursorHasItem ()) {
			...
			Logger.LogDebugMessage ("Drop item to: " + bagNum + "," + slotNum);
			Logger.LogDebugMessage ("Old item: " + item);
			...
			Logger.LogDebugMessage ("Cannot currently use abilities on items");
		} else {
			// Cursor is empty
			if (item != null) {
				_cursorItem = item;
				_cursorAbility = null;
				_UpdateCursor ();
				Logger.LogDebugMessage ("Pickup item: " + item + "," + item.icon);
			}
		}
	}
	...
	public void PickupEquippedItem (int slotNum, AtavismInventoryItem item)
	{
		if (CursorHasItem ()) {
			// Place the item in the slot (possibly taking the item that was in the slot)
			Logger.LogDebugMessage ("Drop item to equipped slot: " + slotNum);
			Logger.LogDebugMessage ("Old item: " + item);
			//_SetContainerItem(containerId, slotId, MarsCursor._cursorItem)
	...
	{
		Logger.LogDebugMessage ("Setting cursor to point");
		// reset our state
		_cursorItem = null;
		_cursorAbility = null;
		_cursorBag = null;
	...
	private void _UpdateCursor ()
	{
		...
		if (_cursorItem != null) {
			...
			Logger.LogDebugMessage ("Setting cursor to item icon: " + _cursorItem.icon);
	...
	public void OnRightClick (bool down, AtavismNode mouseOverObject)
	{
		...
		if (mouseOverObject.PropertyExists ("click_handler")) {
			...
			Logger.LogDebugMessage ("questconcludable");
			...
			Logger.LogDebugMessage ("questavailable");
			...
			Logger.LogDebugMessage ("questinprogress");
			// NetworkAPI.SendQuestInfoRequestMessage (mouseOverObject.Oid);
			if (dist < 6.0)
				ClientAPI.ScriptObject.GetComponent<NpcInteraction> ().GetInteractionOptionsForNpc (mouseOverObject.Oid);
			else
