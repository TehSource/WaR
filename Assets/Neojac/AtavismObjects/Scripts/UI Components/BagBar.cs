			...
			if (i == 0) {
				...
				if (GUI.Button(bagButtonRect, bagData.icon)) {
					...
					if (Event.current.button == 0) {
						...
						Cursor.Instance.PickupBag(bagData);
					}
				}
			} else {
				if (GUI.Button(bagButtonRect, "", GUIStyle.none)) {
					if (Cursor.Instance.CursorHasItem()) {
						Cursor.Instance.PlaceCursorItemAsBag(i);
					} else if (Cursor.Instance.CursorHasBag()) {
						Cursor.Instance.PlaceCursorBag(i);
					}
				}
			}
			if (bagOpen[i]) {
		...
		for (int i = 0; i < bagData.numSlots; i++) {
			...
			if (bagData.items.ContainsKey(i)) {
				if (GUI.Button(buttonRect, bagData.items[i].icon)) {
					if (Event.current.button == Cursor.Instance.PickupButton) {
						if (bagData.items[i].Count > 0)
							Cursor.Instance.PickupBagItem(slot, i, bagData.items[i]);
					} else if (Event.current.button == Cursor.Instance.ActivateButton) {
						// Check for interface overrides
						if (Cursor.Instance.InterfaceStateOverriden()) {
							Cursor.Instance.HandleItemUseOverride(bagData.items[i]);
						} else {
				...
				if (GUI.Button(buttonRect, ""/*, GUIStyle.none*/)) {
					Cursor.Instance.PickupBagItem(slot, i, null);
				}
			}
		}
