...
public class MobController3D : AtavismMobController {
	...
	void Start ()
	{
		...
		if (!_animation) {
			...
			useAnimator = true;
			if (!_animator)
				Logger.LogInfoMessage ("The character you would like to control doesn't have animations. Moving them might look weird.");
			else
				Logger.LogDebugMessage("Got animator: " + _animator);
		}
		
		if (!idleAnimation) {
			_animation = null;
			Logger.LogDebugMessage ("No idle animation found. Turning off animations.");
		}
		if (!walkAnimation) {
			_animation = null;
			Logger.LogDebugMessage ("No walk animation found. Turning off animations.");
		}
		if (!runAnimation) {
			_animation = null;
			Logger.LogDebugMessage ("No run animation found. Turning off animations.");
		}
		if (!jumpPoseAnimation && canJump) {
			_animation = null;
			Logger.LogDebugMessage ("No jump animation found and the character has canJump enabled. Turning off animations.");
		}
		
		moveSpeed = runSpeed;
		
		// Get this mob to ignore collisions with other mobs/players
		foreach (MobNode mNode in ClientAPI.WorldManager.GetMobNodes()) {
			if (mNode.GameObject != null && mNode.GameObject.collider != null && collider != mNode.GameObject.collider)
				Physics.IgnoreCollision (collider, mNode.GameObject.collider);
		}
		//if (go != gameObject)
		if (ClientAPI.GetPlayerObject() != null && !isPlayer)
			Physics.IgnoreCollision (collider, ClientAPI.GetPlayerObject().GameObject.collider);
	}
	...
	void ObjectNodeReady ()
	{
		...
		args[0] = oid.ToString();
		EventSystem.DispatchEvent("MOB_CONTROLLER_ADDED", args);
	}
	
	void OnDestroy () {
		string[] args = new string[1];
		args[0] = oid.ToString();
		EventSystem.DispatchEvent("MOB_CONTROLLER_REMOVED", args);
	...
	public void ApplyJumping ()
	{
		...
		if (IsGrounded () || !followTerrain) {
			...
			if (canJump && Time.time < lastJumpButtonTime + jumpTimeout) {
				...
				Logger.LogDebugMessage ("Applying jump for mob: " + name);
	...
	{
		Logger.LogDebugMessage ("Got damage message for " + name + " with amount: " + damageAmount);
	...
	public override void PlayAnimation(string animationName, float length) {
		if (_animator != null && overrideAnimationName != null && overrideAnimationName != "") {
			Logger.LogDebugMessage("clearing old animation");
			_animator.SetBool (overrideAnimationName, false);
		}
	...
	public void HandleWeaponType (object sender, PropertyChangeEventArgs args)
	{
		...
		weaponType = (string)Client.Instance.WorldManager.GetObjectNode(oid).GetProperty("weaponType");
